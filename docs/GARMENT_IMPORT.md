...
- Mask editor behaviour after a link import: keep the existing auto-open for consistency with uploads, with the `hasAlpha` skip. An explicit "Edit mask" button instead of auto-open was considered and set aside; revisit after use (section 8).
...
| 7 | Web Share Target | Manifest `share_target` and "Import from link" shortcut; `POST /wardrobe/import/share` dispatcher; destination selector for manage-shared wardrobes; README note on platform support. | Dispatcher unit test over `url`/`text`/`title` permutations; manifest shape spec; manual Android checklist in the PR body (WebAPK, manifest lag, cookie on POST). | 1 |
| 7b | Share stash and `returnTo` (follow-up) | Workbox POST route stashing the payload in the Cache API and redirecting to `/wardrobe/new?shared=<id>`; `returnTo` on the login redirect restricted to same-origin paths; pending-import notice on the offline page. | Unit test for the stash handler; manual check with an expired session. | 1 |
...
| Mask editor after a link import | Keep the existing auto-open, with the `hasAlpha` skip | Consistency with uploads; revisit after use. One judge preferred an explicit "Edit mask" button instead. |
