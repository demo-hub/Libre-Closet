...
- `GarmentEnricher` interface with `analyzeImage(jpeg, ctx)` (`normalizeText(text, ctx)` is not built — see the delivery table); providers chosen by a factory on `AI_PROVIDER`, mirroring how `FileModule` picks local or S3. `NullEnricher` when `none`.
...
| 8 | AI enrichment (done for the vision path, opt-in, default off) | `ai/` providers and factory, `POST /wardrobe/import/analyze`, Suggest button (owner-only: absent in a shared wardrobe and refused by the route), provider badge, `PRIVACY_AI_*` copy, README section, `PROJECT_LOG.md` entry recording the opt-in policy. **Not built:** `normalizeText(text, ctx)` and the text schema with `size`/`size_quote` — the page-text enrichment path in §5.7. The extractor already reads a shop page well enough that a model adds little there, and it is the half that would send scraped third-party text to a provider. Deliberately deferred, not forgotten. | Provider specs against a real `http.createServer` stub (Ollama's failure modes are HTTP-shaped) and `jest.spyOn(globalThis, 'fetch')` for Anthropic; schema validation spec; controller spec for 404 when `none`, the analyze success path, and the owner-only refusal; e2e that the button is absent when `none`. | 2 to 3 |
