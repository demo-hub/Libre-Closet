...
| 7b | Share stash and `returnTo` (follow-up) | Workbox POST route stashing the payload in the Cache API and redirecting to `/wardrobe/new?shared=<id>`; `returnTo` on the login redirect restricted to same-origin paths; pending-import notice on the offline page; the destination selector, resolving `ownerId` from the body before the photo is stored. | Unit test for the stash handler; manual check with an expired session. | 1 |
